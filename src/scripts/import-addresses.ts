...
export const importAllStations = async () => {
	...
	if (totalCreated > 0)
		toast.success(`✅ ${totalCreated} Polizeistationen importiert!`);
...
export const showAddressStats = () => {
	...
	for (const city of allCityData) {
		const praesidien = city.addresses.filter(
			(a) => a.type === "praesidium",
		).length;
		...
		console.log(
			`  ${city.name}: ${praesidien} Präsidien, ${reviere} Reviere (${city.addresses.length} total)`,
		);
	}

	console.log(
		`📊 Gesamt: ${totalAddresses} Adressen in ${allCityData.length} Städten`,
	);
...
export const testAPIConnection = async () => {
	try {
		...
		if (response.ok) {
			...
			console.log(
				`✅ API-Verbindung OK. Aktuell ${stations.length} Stationen in der Datenbank.`,
			);
...
export const clearDatabase = async () => {
	try {
		...
		console.log(
			`🗑️ Datenbank geleert: ${stationResult.deletedCount} Stationen, ${addressResult.deletedCount} Adressen gelöscht`,
		);
		return {
			stationCount: stationResult.deletedCount,
			addressCount: addressResult.deletedCount,
		};
	} catch (error) {
