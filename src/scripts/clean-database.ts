...
export const cleanDatabase = async () => {
	...
	try {
		...
		if (deleteStationsResponse.ok) {
			...
			console.warn(
				"⚠️ Fehler beim Löschen der Stationen:",
				deleteStationsResponse.status,
			);
		}
		...
		if (deleteAddressesResponse.ok) {
			...
			console.warn(
				"⚠️ Fehler beim Löschen der Adressen:",
				deleteAddressesResponse.status,
			);
		}
		...
		const stationsCount = stationsResponse.ok
			? (await stationsResponse.json()).length
			: 0;
		const addressesCount = addressesResponse.ok
			? (await addressesResponse.json()).length
			: 0;
		...
		if (stationsCount === 0 && addressesCount === 0) {
			...
			console.log("⚠️ Datenbank teilweise bereinigt:", {
				stationsCount,
				addressesCount,
			});
