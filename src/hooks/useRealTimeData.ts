...
import {
	...
	buildings3DService,
} from "@/lib/services/3d-buildings-service";
import {
	type POI,
	POICategory,
	type POIFilter,
...
} from "@/lib/services/real-time-traffic-service";
...
export function useRealTimeData(
...
): [RealTimeDataState, RealTimeDataActions] {
	...
	const [state, setState] = useState<RealTimeDataState>({
		...
		pois: {
			enabled: Array.isArray(finalConfig.pois)
				? finalConfig.pois.length > 0
				: finalConfig.pois,
			data: [],
			filter: {
				categories: Array.isArray(finalConfig.pois)
					? finalConfig.pois
					: Object.values(POICategory),
			},
			lastUpdate: null,
		},
		buildings3d: {
			enabled:
				finalConfig.buildings3d === true || finalConfig.buildings3d === "auto",
			data: [],
			config: {
				enabled: true,
				minZoom: 15,
	...
	const _updateIntervalRef = useRef<NodeJS.Timeout | null>(null);
	...
	}, [finalConfig.autoConnect, state.traffic.enabled, connect, disconnect]);
	...
	const updateViewport = useCallback(
		(viewport: { bounds: any; zoom: number; center: [number, number] }) => {
	...
	const updatePOIFilter = useCallback(
		(filter: POIFilter) => {
			setState((prev) => ({
				...prev,
				pois: {
	...
	const updateBuildingsConfig = useCallback(
		(config: Partial<BuildingsConfig>) => {
			setState((prev) => ({
				...prev,
				buildings3d: {
					...prev.buildings3d,
