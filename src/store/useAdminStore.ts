...
export const useAdminStore = create<AdminStore>((set, _get) => ({
	...
	loadStations: async (params = { all: true, take: 1000 }) => {
		...
		try {
			...
			console.log(
				"🔍 Stationen Details:",
				data.map((s) => ({ name: s.name, type: s.type, parentId: s.parentId })),
			);
	...
	createStation: async (station) => {
		...
		try {
			...
			console.log(
				"✅ useAdminStore: Stationen nach Erstellung neu geladen:",
				updatedStations.length,
			);
			console.log(
				"🔍 Neue Stationen Details:",
				updatedStations.map((s) => ({
					id: s.id,
					name: s.name,
					type: s.type,
					parentId: s.parentId,
	...
	updateStation: async (id, updates) => {
		...
		try {
			...
			const updatedStations = await fetchStations({ all: true, take: 1000 });
			console.log(
				"✅ useAdminStore: Stationen nach Update neu geladen:",
				updatedStations.length,
			);
			set({ stations: updatedStations, isLoading: false });
		} catch (err) {
	...
	deleteStation: async (id) => {
		...
		try {
			...
			const updatedStations = await fetchStations({ all: true, take: 1000 });
			console.log(
				"✅ useAdminStore: Stationen nach Löschung neu geladen:",
				updatedStations.length,
			);
			set({ stations: updatedStations, isLoading: false });
		} catch (err) {
