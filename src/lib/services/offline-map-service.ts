...
import type { Coordinates, RouteResult } from "../store/app-store";
...
export class OfflineMapService {
	...
	async calculateAlternativeRoutes(
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Alternative routes calculation failed: ${response.statusText}`,
				);
			}
		...
		try {
			...
			if (this.capabilities.geocoding) {
				...
				const response = await fetch(
					`${this.config.geocodingServerUrl}/search?q=${encodeURIComponent(query)}&format=json&limit=5&countrycodes=de`,
				);
			...
			const onlineResponse = await fetch(
				`${this.config.onlineGeocodingUrl}/search?q=${encodeURIComponent(query)}&format=json&limit=5&countrycodes=de`,
				{
					headers: {
	...
	async preloadTiles(
		...
		for (const tile of tiles) {
			try {
				...
				console.warn(
					`Failed to preload tile ${tile.z}/${tile.x}/${tile.y}:`,
					error,
				);
			}
		}
	...
	private async initializeOfflineCapabilities(): Promise<void> {
		try {
			...
			const routingResponse = await fetch(
				`${this.config.routingServerUrl}/route/v1/driving/9.18,48.78;9.19,48.79`,
				{
			...
			const geocodingResponse = await fetch(
				`${this.config.geocodingServerUrl}/search?q=Stuttgart&format=json&limit=1`,
				{
					signal: AbortSignal.timeout(3000),
				},
			);
			...
			const tileResponse = await fetch(
				`${this.config.tileServerUrl}/styles/streets/0/0/0.png`,
				{
					signal: AbortSignal.timeout(3000),
				},
			);
			...
			console.warn(
				"Offline-Capabilities konnten nicht initialisiert werden:",
				error,
			);
		}
	}
	...
	private updateSourceUrlsForOffline(sources: any): any {
		...
		Object.keys(updatedSources).forEach((sourceId) => {
			...
			if (source.url?.startsWith("/api/tiles/")) {
	...
	private async preloadSingleTile(
		style: string,
		z: number,
		x: number,
		y: number,
	...
	private parseOfflineRouteResponse(
		data: any,
		_start: Coordinates,
		end: Coordinates,
	): RouteResult {
	...
	async geocodeOfflineAddress(query: string): Promise<
		Array<{
			lat: number;
			lng: number;
			display_name: string;
	...
	> {
		...
		try {
			...
			const response = await fetch(
				`${this.config.geocodingServerUrl}/search?${params}`,
				{
	...
	private replaceTileUrls(style: any): void {
		if (style.sources) {
			Object.keys(style.sources).forEach((sourceKey) => {
				...
				if (source.url?.includes("maptiler.com")) {
		...
		for (const zoom of zoomLevels) {
			...
			for (const [x, y] of tiles) {
				try {
					...
					console.warn(
						`Tile ${zoom}/${x}/${y} konnte nicht geladen werden:`,
						error,
					);
				}
			}
