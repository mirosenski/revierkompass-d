...
export class POIStreamingService {
  ...
  async loadPOIs(viewport: POIViewport, filter: POIFilter = this.currentFilter): Promise<POI[]> {
  ...
  private async fetchPOIsForViewport(viewport: POIViewport, filter: POIFilter): Promise<POI[]> {
    ...
    for (const category of filter.categories) {
      try {
        ...
        const pois = await this.fetchPOIsForCategory(bounds, categoryConfig, maxPOIsPerCategory);
        allPOIs.push(...pois);
      } catch (error) {
  ...
  private async fetchPOIsForCategory(
    ...
    const query = this.buildOverpassQuery(bounds, categoryConfig.query, maxResults);
    ...
    return new Promise((resolve, reject) => {
      this.requestQueue.push(async () => {
        try {
          ...
          console.error(`📍 Fehler beim Laden von ${categoryConfig.name}:`, error);
          reject(error);
        }
      });
    });
  ...
  private parseOverpassResponse(data: any, categoryConfig: POICategoryConfig): POI[] {
    const pois: POI[] = [];
    ...
    for (const element of data.elements) {
      if (element.type === 'node' && element.lat && element.lon) {
        const poi: POI = {
          id: `poi-${element.id}`,
          type: element.type,
          name: element.tags?.name || element.tags?.ref || `Unbekannt ${categoryConfig.name}`,
  ...
  private notifyListeners(data: POI[]): void {
    this.listeners.forEach(listener => {
      try {
        ...
        console.error('📍 Fehler beim Benachrichtigen des POI-Listeners:', error);
      }
    });
  }
  ...
  generateMockPOIs(bounds: { north: number; south: number; east: number; west: number }): POI[] {
    const mockPOIs: POI[] = [];
