...
class RoutingService {
	...
	public async checkOfflineRouting(): Promise<void> {
		try {
			...
			const response = await fetch(
				`${this.OFFLINE_ROUTING_URL}/route/v1/driving/9.18,48.78;9.19,48.79?overview=false`,
				{
	...
	async calculateMultipleRoutes(
		...
		const destinations = [
			...
			...selectedCustomAddressIds.map((id) => ({
				id,
				...
				try {
					...
					if (route) {
						return {
							...
							estimatedFuel: this.calculateFuelConsumption(
								route.distance / 1000,
								routingProfile,
							),
							...
							stationType:
								type === "station" ? (destination as any).type : undefined,
						};
					}
				} catch (error) {
					console.error(
						`Fehler bei Routenberechnung zu ${destination.name}:`,
						error,
					);
	...
	async calculateSingleRoute(
		...
		for (const provider of this.ROUTING_PROVIDERS) {
			try {
				...
				switch (provider.name) {
					...
					case "OSRM-Alt":
					case "OSRM-DE":
						route = await this.calculateWithOSRM(
							start,
							end,
							provider.url,
							options.profile,
						...
						break;
					case "Valhalla":
						route = await this.calculateWithValhalla(
							start,
							end,
							options.profile,
						);
						break;
					case "GraphHopper":
						route = await this.calculateWithGraphHopper(
							start,
							end,
							options.profile,
						);
		...
		const fallbackRoute = this.createRealisticFallbackRoute(
			start,
			end,
			options.profile,
		);
	...
	private parseGraphHopperResponse(data: any): RouteResponse {
		...
		const coordinates = path.points.coordinates.map((coord: number[]) => [
			coord[0],
			coord[1],
		]);
	...
	private mapProfileToRouteType(
		profile: string,
	): "Schnellste" | "Kürzeste" | "Ökonomisch" {
	...
	private calculateFuelConsumption(distance: number, profile: string): number {
		...
		const multiplier =
			profile === "eco" ? 0.8 : profile === "fastest" ? 1.2 : 1.0;
	...
	private createFallbackRoute(
		...
		return {
			...
			route: {
				coordinates: this.generateRealisticRoute(
					startAddress.coordinates,
					destination.coordinates,
					roadDistance,
				),
