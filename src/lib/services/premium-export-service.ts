...
class PremiumExportService {
	...
	async exportToPremiumExcel(
		...
		try {
			...
			// Statistics sheet
			const statsWs = this.createStatisticsSheet(
				routeResults,
				allStations,
				customAddresses,
			);
	...
	private prepareExportData(
		...
		return routeResults.map((result) => {
			...
			if (result.destinationType === "station") {
				...
				const customAddr = customAddresses.find(
					(a) => a.id === result.destinationId,
				);
				destinationInfo = {
					name: result.destinationName,
					type: "Eigene Adresse",
					address: customAddr
						? `${customAddr.street}, ${customAddr.zipCode} ${customAddr.city}`
						: "",
	...
	private createStatisticsSheet(
		...
		cities.forEach((city) => {
			...
			if (cityRoutes.length > 0) {
				cityStats.push({
					...
					Naechstes_Ziel: cityRoutes.sort((a, b) => a.distance - b.distance)[0]
						.destinationName,
				});
			}
		});
		...
		const ws = XLSX.utils.json_to_sheet(
			cityStats.sort(
				(a, b) =>
					parseFloat(a.Durchschnittliche_Entfernung_km) -
					parseFloat(b.Durchschnittliche_Entfernung_km),
			),
		);
	...
	async exportToPremiumPDF(
		...
		try {
			...
			pdf.setFontSize(14);
			pdf.text("Polizei Baden-Württemberg", pageWidth / 2, 20, {
				align: "center",
			});
