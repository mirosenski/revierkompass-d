...
export class Buildings3DService {
  private buildingsCache: Map<string, { data: Building3D[]; timestamp: number }> = new Map();
  ...
  async loadBuildings(viewport: BuildingsViewport): Promise<Building3D[]> {
    ...
    // Performance-Check
    if (this.config.performanceCheck && !this.performanceMonitor.canLoadBuildings()) {
      console.warn('🏢 Performance-Check fehlgeschlagen - 3D-Buildings deaktiviert');
      return [];
    }
    ...
    try {
      ...
      console.log(`🏢 ${buildings.length} 3D-Buildings geladen für Zoom ${viewport.zoom}`);
      return buildings;
    } catch (error) {
  ...
  private shouldLoadBuildings(viewport: BuildingsViewport): boolean {
    ...
    if (viewport.zoom < this.config.minZoom || viewport.zoom > this.config.maxZoom) {
      return false;
    }
  ...
  private isCityInWhitelist(center: [number, number]): boolean {
    ...
    return lat >= bwBounds.south && lat <= bwBounds.north && 
           lng >= bwBounds.west && lng <= bwBounds.east;
  }

  private async fetchBuildingsForViewport(viewport: BuildingsViewport): Promise<Building3D[]> {
  ...
  private generateMockBuildings(bounds: { north: number; south: number; east: number; west: number }, lodLevel: LODLevel): Building3D[] {
  ...
  getMapStyleConfig(zoom: number): any {
    ...
    return {
      ...
      'fill-extrusion-color': [
        'case',
        ['==', ['get', 'building'], 'residential'], '#e5e7eb',
        ['==', ['get', 'building'], 'commercial'], '#fbbf24',
        ['==', ['get', 'building'], 'industrial'], '#6b7280',
        '#d1d5db' // Default
      ],
      'fill-extrusion-translate': [0, 0],
      'fill-extrusion-translate-anchor': 'map',
      'fill-extrusion-pattern': lodLevel.detail === 'textured' ? 'building-pattern' : undefined
    };
  }
  ...
  private notifyListeners(data: Building3D[]): void {
    this.listeners.forEach(listener => {
      try {
        ...
        console.error('🏢 Fehler beim Benachrichtigen des Buildings-Listeners:', error);
      }
    });
  }
...
class PerformanceMonitor {
  ...
  canLoadBuildings(): boolean {
    ...
    console.log(`🏢 Performance-Check: FPS=${avgFps.toFixed(1)}, Memory=${avgMemory.toFixed(1)}MB`);
