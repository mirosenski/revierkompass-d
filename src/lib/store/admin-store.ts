...
export const useAdminStore = create<AdminState>((set, get) => ({
	...
	updateStation: async (id, updates) => {
		...
		try {
			...
			set({
				error:
					error instanceof Error ? error.message : "Error updating station",
				isLoading: false,
			});
		}
	},
	...
	deleteStation: async (id) => {
		...
		try {
			const currentStations = get().allStations;
			const updatedStations = currentStations.filter(
				(station) => station.id !== id,
			);

			set({
				allStations: updatedStations,
				selectedStations: get().selectedStations.filter(
			...
			set({
				error:
					error instanceof Error ? error.message : "Error deleting station",
				isLoading: false,
			});
		}
	},
	...
	deleteMultipleStations: async (ids) => {
		...
		try {
			...
			set({
				error:
					error instanceof Error ? error.message : "Error deleting stations",
				isLoading: false,
			});
		}
	},
	...
	importStations: async (stations) => {
		...
		try {
			...
			set({
				error:
					error instanceof Error ? error.message : "Error importing stations",
				isLoading: false,
			});
		}
	},
	...
	loadStats: async () => {
		...
		try {
			...
			stats.totalPrecincts = currentStations.filter(
				(s) => s.type === "praesidium",
			).length;
			stats.totalStationHouses = currentStations.filter(
				(s) => s.type === "revier",
			).length;
	...
	updateStats: () => {
		...
		if (currentStats) {
			const updatedStats: AdminStats = {
				...
				totalPrecincts: currentStations.filter((s) => s.type === "praesidium")
					.length,
				totalStationHouses: currentStations.filter((s) => s.type === "revier")
					.length,
				lastUpdate: new Date(),
			};
