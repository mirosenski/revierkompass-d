...
export const fetchStations = async (params = {}): Promise<Station[]> => {
	try {
		...
		try {
			...
			console.warn(
				"⚠️ Backend Health Check fehlgeschlagen:",
				healthError.message,
			);
		}
	...
	try {
		...
		const stationData = {
			...
			coordinates: Array.isArray(station.coordinates)
				? station.coordinates
				: [
						(station.coordinates as any).lat || 0,
						(station.coordinates as any).lng || 0,
					],
		};
...
export const geocodeAddress = async (query: string): Promise<any[]> => {
	try {
		...
		try {
			...
			console.log(
				"⚠️ Lokaler Nominatim nicht verfügbar, verwende Online-Fallback",
			);
		}
