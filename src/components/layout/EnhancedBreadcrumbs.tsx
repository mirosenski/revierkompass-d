...
const EnhancedBreadcrumbs: React.FC<EnhancedBreadcrumbsProps> = ({
	...
	const handleBreadcrumbClick = (item: BreadcrumbItem) => {
		...
		if (item.view && item.step !== undefined) {
			...
			if (item.step === 1) {
				console.log(
					"🔄 Breadcrumb: Klick auf Adresse - Wizard wird komplett zurückgesetzt",
				);
				try {
					// Beide Stores zurücksetzen
					const { resetWizard } =
						require("@/lib/store/app-store").useAppStore.getState();
	...
	const mobileBreadcrumbs = React.useMemo(() => {
		...
		const previous =
			breadcrumbs[breadcrumbs.findIndex((item) => item.active) - 1];
