...
export const StationModal: React.FC<StationModalProps> = React.memo(
	({
		...
		const [formData, setFormData] =
			useState<StationFormData>(INITIAL_FORM_DATA);
		...
		const handleSubmit = useCallback(
			async (e: React.FormEvent) => {
				...
				try {
					...
					toast.success(
						station
							? "Station erfolgreich aktualisiert"
							: "Station erfolgreich erstellt",
					);
				} catch (err) {
		...
		const handleInputChange = useCallback(
			(field: keyof StationFormData, value: any) => {
				setFormData((prev) => ({
					...prev,
					[field]: value,
				}));
					...
					<form onSubmit={handleSubmit} className="p-6 space-y-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div>
								...
								>
									<option key="praesidium" value="praesidium">
										Präsidium
									</option>
									<option key="revier" value="revier">
										Revier
									</option>
								</select>
							</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="flex items-center gap-3">
								<input
									...
									onChange={(e) =>
										handleInputChange("notdienst24h", e.target.checked)
									}
