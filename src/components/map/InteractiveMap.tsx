...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
	...
	const [_trafficEnabled, _setTrafficEnabled] = useState(false);
	const [_clustersEnabled, _setClustersEnabled] = useState(true);
	...
	}, [routeResults, startCoordinates.lat, startCoordinates.lng]);
    ...
    `;
      ...
      `),
		...
		routeResults.forEach((route) => {
			...
			const iconColor =
				route.destinationType === "station"
					? route.destinationName.includes("Präsidium")
						? "#8b5cf6"
						: "#3b82f6"
					: "#f97316";
	...
	}, [routeResults, startCoordinates.lat, startCoordinates.lng, startAddress, onMarkerClick]);

	const getIconSVG = (iconType: string) => {
		const icons = {
			badge:
				'<path d="M8.252 4.75H5.5a.75.75 0 0 0-.75.75v9.5c0 .414.336.75.75.75h13a.75.75 0 0 0 .75-.75v-9.5a.75.75 0 0 0-.75-.75h-2.752M8.252 4.75a.75.75 0 0 1 .75-.75h6a.75.75 0 0 1 .75.75v.5a.75.75 0 0 1-.75.75h-6a.75.75 0 0 1-.75-.75v-.5Z"/>',
	...
	const showRoutePopup = (route: RouteResult, lngLat: any) => {
		...
		const _popup = new Popup({ closeButton: true, closeOnClick: false })
			.setLngLat(lngLat)
			.setHTML(`
					...
					<div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
						...
						<div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
							<div className="text-lg font-bold text-orange-600">
								{routeResults
									.reduce((sum, r) => sum + r.estimatedFuel, 0)
									.toFixed(1)}
							</div>
							<div className="text-xs text-gray-600 dark:text-gray-400">
								Liter
										...
										<div className="flex items-center space-x-2 flex-1 min-w-0">
											...
											<div className="flex-1 min-w-0">
												...
												<div className="text-xs text-gray-500 dark:text-gray-400">
													...
													{route.stationType || route.provider}
												</div>
											</div>
										</div>
									</div>
