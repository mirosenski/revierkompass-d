...
import { TRAFFIC_LEVELS, TrafficSegment } from '@/lib/services/real-time-traffic-service';
import { POI, POICategory, POI_CATEGORIES } from '@/lib/services/poi-streaming-service';
import { Building3D } from '@/lib/services/3d-buildings-service';
...
const RealTimeMapComponent: React.FC<RealTimeMapComponentProps> = ({
  ...
  useEffect(() => {
    ...
    if (trafficSource) {
      ...
      map.current.setPaintProperty('traffic-lines', 'line-color', [
        'case',
        ['==', ['get', 'level'], 0], TRAFFIC_LEVELS[0].color,
        ['==', ['get', 'level'], 1], TRAFFIC_LEVELS[1].color,
        ['==', ['get', 'level'], 2], TRAFFIC_LEVELS[2].color,
        ['==', ['get', 'level'], 3], TRAFFIC_LEVELS[3].color,
        '#cccccc'
      ]);

      map.current.setPaintProperty('traffic-lines', 'line-opacity', [
        'case',
        ['==', ['get', 'level'], 0], TRAFFIC_LEVELS[0].opacity,
        ['==', ['get', 'level'], 1], TRAFFIC_LEVELS[1].opacity,
        ['==', ['get', 'level'], 2], TRAFFIC_LEVELS[2].opacity,
        ['==', ['get', 'level'], 3], TRAFFIC_LEVELS[3].opacity,
        0.6
      ]);
    }
  ...
  }, [realTimeState.pois.data, mapLoaded, realTimeState.pois.enabled, realTimeState.pois.filter]);

  // 3D Buildings Data Update
  useEffect(() => {
    if (!map.current || !mapLoaded || !realTimeState.buildings3d.enabled) return;
  ...
  }, [realTimeState.buildings3d.data, mapLoaded, realTimeState.buildings3d.enabled]);
  ...
  const showRoutePopup = (route: RouteResult) => {
    ...
    const centerLng = (coordinates[0][0] + coordinates[coordinates.length - 1][0]) / 2;
      ...
      {showRealTimeControls && (
        <div className="absolute top-4 right-4 space-y-3">
          ...
          <RealTimeToggle
            ...
            connectionQuality={realTimeState.traffic.connected ? 'excellent' : 'disconnected'}
          />
