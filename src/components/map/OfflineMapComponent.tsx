...
const OfflineMapComponent: React.FC<OfflineMapComponentProps> = ({
	...
	const [offlineCapabilities, setOfflineCapabilities] =
		useState<OfflineCapabilities | null>(null);
	const [isPreloadingTiles, setIsPreloadingTiles] = useState(false);
	const [preloadProgress, setPreloadProgress] = useState(0);
	const [showOfflinePanel, setShowOfflinePanel] = useState(false);
	const [availableProfiles, setAvailableProfiles] = useState<any[]>([]);
	const [_nbanData, setNbanData] = useState<any>(null);
	...
	}, [mapLoaded, routeResults, startCoordinates, startAddress]);
	...
	const recalculateRoute = async (routeId: string, profileId: string) => {
		...
		try {
			...
			toast.success("Route erfolgreich neu berechnet", {
				id: `route-${routeId}`,
			});
		} catch (error) {
			console.error("Route recalculation failed:", error);
			toast.error("Fehler bei der Routenneuberechnung", {
				id: `route-${routeId}`,
			});
		}
	};
