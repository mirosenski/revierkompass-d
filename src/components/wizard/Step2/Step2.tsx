...
const Step2: React.FC = () => {
	const { getStationsByType, getReviereByPraesidium, loadStations, stations } =
		useStationStore();
	const { setSelectedStations, setSelectedCustomAddresses } = useWizardStore();
	const {
		setSelectedStations: setAppSelectedStations,
		setSelectedCustomAddresses: setAppSelectedCustomAddresses,
	} = useAppStore();
	...
	const handleEditPermanent = async (address: any) => {
		try {
			...
			try {
				...
				console.warn(
					"Backend nicht verfügbar, bearbeite nur lokal:",
					backendError,
				);
	...
	const handleVoiceCommand = () => {
		if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
			...
			recognition.onresult = (event: any) => {
				...
				if (
					transcript.includes("stuttgart") ||
					transcript.includes("stuttgarter")
				) {
					...
					setSelectedStations([
						...new Set([...selectedStations, ...stuttgartStations]),
					]);
					toast.success("Alle Stuttgarter Stationen ausgewählt");
				} else if (
					transcript.includes("alle") ||
					transcript.includes("select all")
				) {
					// Alle sichtbaren Stationen auswählen
					const allVisible = praesidiumWithReviere.flatMap((p) => [
						p.id,
						...p.reviere.map((r) => r.id),
					]);
					setSelectedStations([
						...new Set([...selectedStations, ...allVisible]),
					]);
					toast.success("Alle Stationen ausgewählt");
				} else if (
					transcript.includes("nichts") ||
	...
	const handleCommandExecute = (commandKey: string) => {
		const commands = [
			...
			},
			{
				key: "alle",
				action: () => {
					const allStations = praesidiumWithReviere.flatMap((p) => [
						p.id,
						...p.reviere.map((r) => r.id),
					]);
					setSelectedStations([
						...new Set([...selectedStations, ...allStations]),
					]);
					toast.success("Alle Stationen ausgewählt");
				},
			},
	...
	}, [handleCommandDialog, handleVoiceCommand]);
	...
	const selectedPraesidien = praesidiumWithReviere.filter((p) =>
		selectedStations.includes(p.id),
	);
					...
					<div className="flex items-center space-x-4">
						...
						<div>
							<h2 className="text-2xl font-bold text-gray-900 dark:text-white">
								{totalSelected === 0
									? "Keine Ziele ausgewählt"
									: `${totalSelected} Ziele ausgewählt`}
							</h2>
				...
				{totalSelected > 0 && (
					<div className="mt-6">
						<div className="flex justify-between text-sm text-gray-600 dark:text-gray-400 mb-2">
							...
							<span>
								{totalSelected} von{" "}
								{praesidiumWithReviere.length + selectedCustomCount} möglichen
								Zielen
							</span>
						</div>
						<div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
							<motion.div
								...
								initial={{ width: 0 }}
								animate={{
									width: `${Math.min((totalSelected / (praesidiumWithReviere.length + selectedCustomCount)) * 100, 100)}%`,
								}}
