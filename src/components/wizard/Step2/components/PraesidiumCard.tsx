...
import { Building, CheckCircle2, ChevronDown } from "lucide-react";
import type React from "react";
import { animationVariants } from "../animations/variants";
import type { PraesidiumCardProps } from "../types";
...
const PraesidiumCard: React.FC<PraesidiumCardProps> = ({
	...
	const selectedReviere = praesidium.reviere.filter((r) =>
			...
			{viewMode === "list" && (
				<div className="mb-4">
					<div className="flex justify-between text-xs text-gray-500 mb-1">
						...
						<span>
							{selectedReviere.length}/{praesidium.reviere.length}
						</span>
					</div>
					<div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
						<div
							...
							style={{
								width: `${(selectedReviere.length / praesidium.reviere.length) * 100}%`,
							}}
						/>
					</div>
				</div>
