...
import {
	type Station as AppStoreStation,
	type RouteResult,
	useAppStore,
} from "@/lib/store/app-store";
import { useStationStore } from "@/store/useStationStore";
import { useWizardStore } from "@/store/useWizardStore";
import type { Station as StationType } from "@/types/station.types";
...
const Step3PremiumExport: React.FC = () => {
	...
	const exportToExcel = () => {
		...
		const worksheetData = [
			...
			[
				"Gesamtkraftstoff (L):",
				results.reduce((sum, r) => sum + r.estimatedFuel, 0).toFixed(1),
			],
			[
				"Gesamtkosten (€):",
	...
	const _totalFuel = results.reduce((sum, r) => sum + r.estimatedFuel, 0);
				...
				<div className="flex items-start space-x-3">
					...
					<div>
						...
						<p className="text-blue-800 dark:text-blue-200 text-sm mb-4">
							Diese Karte bietet erweiterte Offline-Funktionen speziell für den
							Einsatz in Baden-Württemberg. Alle Kartendaten, Routing-Profile
							und NBAN-Zonen sind lokal verfügbar.
						</p>
			...
			{isCalculating ? (
				...
				>
					...
					<p className="text-blue-100">
						Multi-Provider Routing wird durchgeführt (OSRM, Valhalla,
						GraphHopper)
					</p>
