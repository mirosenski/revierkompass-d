...
export const createAllUlmAddresses = async () => {
	...
	try {
		...
		const praesidium = ulmAddresses.find(
		...
		if (praesidium) {
			try {
				...
				toast.loading(
					`Erstelle Ulm-Stationen... (${createdCount}/${ulmAddresses.length})`,
					{ id: loadingToast },
				);
		...
		if (createdCount > 0) {
			toast.success(
				`✅ ${createdCount} Stationen für Polizeipräsidium Ulm erfolgreich erstellt!${errorCount > 0 ? ` (${errorCount} Fehler)` : ""}`,
			);
		} else {
