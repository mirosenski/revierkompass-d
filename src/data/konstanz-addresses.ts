...
export const createAllKonstanzAddresses = async () => {
	...
	try {
		...
		const praesidium = konstanzAddresses.find(
		...
		if (praesidium) {
			try {
				...
				toast.loading(
					`Erstelle Konstanz-Stationen... (${createdCount}/${konstanzAddresses.length})`,
					{ id: loadingToast },
				);
		...
		if (createdCount > 0) {
			toast.success(
				`✅ ${createdCount} Stationen für Polizeipräsidium Konstanz erfolgreich erstellt!${errorCount > 0 ? ` (${errorCount} Fehler)` : ""}`,
			);
		} else {
