...
export const createAllPforzheimAddresses = async () => {
	...
	try {
		...
		const praesidium = pforzheimAddresses.find(
		...
		if (praesidium) {
			try {
				...
				toast.loading(
					`Erstelle Pforzheim-Stationen... (${createdCount}/${pforzheimAddresses.length})`,
					{ id: loadingToast },
				);
		...
		if (createdCount > 0) {
			toast.success(
				`✅ ${createdCount} Stationen für Polizeipräsidium Pforzheim erfolgreich erstellt!${errorCount > 0 ? ` (${errorCount} Fehler)` : ""}`,
			);
		} else {
