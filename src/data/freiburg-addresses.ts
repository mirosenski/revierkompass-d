...
export const createAllFreiburgAddresses = async () => {
	...
	try {
		...
		const praesidium = freiburgAddresses.find(
		...
		if (praesidium) {
			try {
				...
				toast.loading(
					`Erstelle Freiburg-Stationen... (${createdCount}/${freiburgAddresses.length})`,
					{ id: loadingToast },
				);
		...
		if (createdCount > 0) {
			toast.success(
				`✅ ${createdCount} Stationen für Polizeipräsidium Freiburg erfolgreich erstellt!${errorCount > 0 ? ` (${errorCount} Fehler)` : ""}`,
			);
		} else {
