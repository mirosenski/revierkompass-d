...
export const createAllMannheimAddresses = async () => {
	...
	try {
		...
		const praesidium = mannheimAddresses.find(
		...
		if (praesidium) {
			try {
				...
				toast.loading(
					`Erstelle Mannheim-Stationen... (${createdCount}/${mannheimAddresses.length})`,
					{ id: loadingToast },
				);
		...
		if (createdCount > 0) {
			toast.success(
				`✅ ${createdCount} Stationen für Polizeipräsidium Mannheim erfolgreich erstellt!${errorCount > 0 ? ` (${errorCount} Fehler)` : ""}`,
			);
		} else {
