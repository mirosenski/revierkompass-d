...
export const createAllAalenAddresses = async () => {
	...
	try {
		...
		const praesidium = aalenAddresses.find(
		...
		if (praesidium) {
			try {
				...
				toast.loading(
					`Erstelle Aalen-Stationen... (${createdCount}/${aalenAddresses.length})`,
					{ id: loadingToast },
				);
		...
		if (createdCount > 0) {
			toast.success(
				`✅ ${createdCount} Stationen für Polizeipräsidium Aalen erfolgreich erstellt!${errorCount > 0 ? ` (${errorCount} Fehler)` : ""}`,
			);
		} else {
