...
function App() {
	const [currentView, setCurrentView] = useState<
		"wizard" | "login" | "admin" | "test" | "logo-test"
	>("wizard");
	...
	useEffect(() => {
		console.log("🚀 App: Navigation basierend auf Auth-Status:", {
			isAuthenticated,
			isAdmin,
		});
	...
	useEffect(() => {
		if (isAuthenticated && isAdmin && currentView === "login") {
			setCurrentView("admin");
			console.log(
				"🔄 Automatische Navigation zum Admin-Bereich nach Anmeldung",
			);
		}
	}, [isAuthenticated, isAdmin, currentView]);

	const handleAdminLogin = () => {
		console.log("🔐 handleAdminLogin aufgerufen:", {
			isAuthenticated,
			isAdmin,
			currentView,
		});
		...
		if (isAuthenticated && isAdmin) {
			console.log(
				"🔄 Bereits angemeldet - direkte Navigation zum Admin-Bereich",
			);
			setCurrentView("admin");
		} else {
	...
	const handleBackToWizard = () => {
		...
		try {
			...
			try {
				const { resetWizard: resetWizardStore } =
					require("@/store/useWizardStore").useWizardStore.getState();
			...
			console.log(
				"✅ App: Wizard komplett zurückgesetzt - alle Steps und Auswahlen gelöscht",
			);
		} catch (error) {
	...
	const handleGoToAdmin = () => {
		console.log("🔐 handleGoToAdmin aufgerufen:", {
			isAuthenticated,
			isAdmin,
			currentView,
		});
